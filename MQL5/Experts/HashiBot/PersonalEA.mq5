...
bool ExecuteSelectedPlan(const TradePlan &plan,const MarketContext &ctx,const RiskDecision &risk,const string symbol,const double score,const bool planOk,const bool riskApproved,const bool portfolioApproved,TradeState &tstate,string &reason){ bool testerMode=(MQLInfoInteger(MQL_TESTER)>0); long terminalTradeAllowed=TerminalInfoInteger(TERMINAL_TRADE_ALLOWED),mqlTradeAllowed=MQLInfoInteger(MQL_TRADE_ALLOWED),accountTradeAllowed=AccountInfoInteger(ACCOUNT_TRADE_ALLOWED),symbolTradeMode=-1; SymbolInfoInteger(symbol,SYMBOL_TRADE_MODE,symbolTradeMode); bool permissionAllowed=(testerMode || (terminalTradeAllowed>0 && mqlTradeAllowed>0 && accountTradeAllowed>0 && symbolTradeMode!=SYMBOL_TRADE_MODE_DISABLED)); Print(StringFormat("[MT5_PERMISSION_CHECK] tester=%s terminalTradeAllowed=%d mqlTradeAllowed=%d accountTradeAllowed=%d symbolTradeMode=%d allowed=%s reason=%s",(testerMode?"true":"false"),(int)terminalTradeAllowed,(int)mqlTradeAllowed,(int)accountTradeAllowed,(int)symbolTradeMode,(permissionAllowed?"true":"false"),(permissionAllowed?"none":"mt5_trade_permission_denied"))); double minLot=SymbolInfoDouble(symbol,SYMBOL_VOLUME_MIN),maxLot=SymbolInfoDouble(symbol,SYMBOL_VOLUME_MAX),lotStep=SymbolInfoDouble(symbol,SYMBOL_VOLUME_STEP); double ask=SymbolInfoDouble(symbol,SYMBOL_ASK),bid=SymbolInfoDouble(symbol,SYMBOL_BID),entryPx=(plan.direction==TRADE_DIR_LONG?ask:bid); double volume=MathMax(minLot,MathMin(maxLot,risk.approvedLots)); if(lotStep>0.0) volume=MathFloor(volume/lotStep)*lotStep; volume=NormalizeDouble(volume,2); int stops=(int)SymbolInfoInteger(symbol,SYMBOL_TRADE_STOPS_LEVEL); double stopLevel=stops*ctx.point; bool sideOk=((plan.direction==TRADE_DIR_LONG&&plan.stopLoss<entryPx&&plan.takeProfit1>entryPx)||(plan.direction==TRADE_DIR_SHORT&&plan.stopLoss>entryPx&&plan.takeProfit1<entryPx)); bool stopOk=(MathAbs(entryPx-plan.stopLoss)>=stopLevel && MathAbs(plan.takeProfit1-entryPx)>=stopLevel); double margin=0.0; bool marginOk=OrderCalcMargin((plan.direction==TRADE_DIR_LONG?ORDER_TYPE_BUY:ORDER_TYPE_SELL),symbol,volume,entryPx,margin) && (AccountInfoDouble(ACCOUNT_MARGIN_FREE)<=0.0 || margin<=AccountInfoDouble(ACCOUNT_MARGIN_FREE)); bool payloadOk=(StringLen(symbol)>0 && (plan.direction==TRADE_DIR_LONG||plan.direction==TRADE_DIR_SHORT) && plan.entryPrice>0.0 && plan.stopLoss>0.0 && plan.takeProfit1>0.0 && volume>=minLot && volume<=maxLot && sideOk && stopOk && marginOk && ctx.spreadPoints<=MaxSpreadPoints); Print(StringFormat("[ORDER_PAYLOAD_CHECK] symbol=%s type=%s volume=%.2f minLot=%.2f maxLot=%.2f lotStep=%.2f entry=%.5f sl=%.5f tp=%.5f stopLevel=%.5f marginOk=%s payloadOk=%s reason=%s",symbol,(plan.direction==TRADE_DIR_LONG?"BUY":"SELL"),volume,minLot,maxLot,lotStep,entryPx,plan.stopLoss,plan.takeProfit1,stopLevel,(marginOk?"true":"false"),(payloadOk?"true":"false"),(payloadOk?"none":"payload_invalid"))); if(!(planOk&&riskApproved&&portfolioApproved&&permissionAllowed&&payloadOk)){ reason=(!planOk?"plan_invalid":(!riskApproved?"risk_not_approved":(!portfolioApproved?"portfolio_not_approved":(!permissionAllowed?"mt5_trade_permission_denied":"payload_invalid")))); Print(StringFormat("[FINAL_SUBMIT_BLOCKER] blocker=%s reason=%s file=PersonalEA.mq5 function=ExecuteSelectedPlan stage=pre_ordermanager",reason,reason)); if(testerMode && planOk && riskApproved && portfolioApproved && payloadOk) Print(StringFormat("[CRITICAL_SELECTED_NOT_SUBMITTED] selected=true planOk=%s riskApproved=%s portfolioApproved=%s payloadOk=%s lastGate=permission reason=%s",(planOk?"true":"false"),(riskApproved?"true":"false"),(portfolioApproved?"true":"false"),(payloadOk?"true":"false"),reason)); return false; } Print(StringFormat("[ORDERMANAGER_CALL] symbol=%s strategy=%s type=%s lot=%.2f entry=%.5f sl=%.5f tp=%.5f riskPct=%.2f score=%.2f rr=%.2f",symbol,StrategyName(plan.strategy),(plan.direction==TRADE_DIR_LONG?"BUY":"SELL"),volume,entryPx,plan.stopLoss,plan.takeProfit1,g_risk.RiskPercent(),score,(MathAbs(entryPx-plan.stopLoss)>0.0?MathAbs(plan.takeProfit1-entryPx)/MathAbs(entryPx-plan.stopLoss):0.0))); RiskDecision sendRisk=risk; sendRisk.approvedLots=volume; string execReason=""; g_testerOrdersAttempted++; g_starveOrderManagerReached++; bool submitted=g_order.Submit(plan, sendRisk, ctx, (testerMode?EXEC_MODE_TESTER_SIM:executionMode), true, false, false, true, MagicNumber, maxSlippagePoints, TradeCommentPrefix, tstate, execReason); Print(StringFormat("[ORDERMANAGER_RESULT] success=%s retcode=%d retcodeDescription=%s ticket=%I64d lastError=%d",(submitted?"true":"false"),(int)g_order.LastRetcode(),g_order.LastRetcodeDescription(),g_order.LastOrder(),GetLastError())); reason=(submitted?"submitted":execReason); return submitted; }
...
void ProcessSymbol(const string symbol,const bool isNewBar)
  {
   ...
   if(risk.approved && portfolioOK && preSubmitFieldsOk && submitAllowed)
     {
      Print(StringFormat("[SUBMIT_GATE_DIAG] selected=true planValid=%s planOk=%s riskApproved=%s portfolioApproved=%s submitAllowed=%s dryRunOnly=%s signalOnly=%s testerMode=%s executionMode=%d accountMode=%s rejectReason=none finalAction=call_ordermanager",
                         ...
                         (submitExecutionMode==EXEC_MODE_DRYRUN?"true":"false"),(submitExecutionMode==EXEC_MODE_LOG_ONLY?"true":"false"),(testerMode?"true":"false"),(int)submitExecutionMode,accountModeLabel));
      string execReason="";
      bool submitted=ExecuteSelectedPlan(chosenPlan, ctx, risk, symbol, chosenScore, planOk, risk.approved, portfolioOK, tstate, execReason);
...
int OnInit(){ if(enableDryRunSelfCheck){} g_ctxBuilder.Init(); g_regime.Init(); g_arb.Init(PROFILE_PERSONAL); g_arb.Configure(EnableSecondaryStrategy,EnableArbitrator); g_risk.Init(PROFILE_PERSONAL);
   ...
   Print("[BUILD_SIGNATURE] PersonalEA build=EXECUTION_SPINE_FIX_2026_05_20 executionSpine=ON permissionCheck=ON orderPayloadCheck=ON testerSubmitRequired=ON");
