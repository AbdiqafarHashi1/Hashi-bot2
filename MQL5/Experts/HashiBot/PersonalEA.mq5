   ...
   if(risk.approved && portfolioOK && preSubmitFieldsOk && submitAllowed)
     {
      Print(StringFormat("[SUBMIT_GATE_DIAG] selected=true planValid=%s planOk=%s riskApproved=%s portfolioApproved=%s submitAllowed=%s dryRunOnly=%s signalOnly=%s testerMode=%s executionMode=%d accountMode=%s rejectReason=none finalAction=call_ordermanager",
                         ...
                         (submitExecutionMode==EXEC_MODE_DRYRUN?"true":"false"),(submitExecutionMode==EXEC_MODE_LOG_ONLY?"true":"false"),(testerMode?"true":"false"),(int)submitExecutionMode,accountModeLabel));
      string execReason="";
      bool submitted=false; g_testerOrdersAttempted++; g_starveOrderManagerReached++;
      Print(StringFormat("[ORDERMANAGER_CALL] symbol=%s strategy=%s dir=%s entry=%.5f sl=%.5f tp1=%.5f tp2=%.5f lots=%.2f riskPct=%.2f magic=%d comment=%s",
                         symbol,StrategyName(chosenPlan.strategy),DirName(chosenPlan.direction),chosenPlan.entryPrice,chosenPlan.stopLoss,chosenPlan.takeProfit1,chosenPlan.takeProfit2,risk.approvedLots,g_risk.RiskPercent(),(int)MagicNumber,TradeCommentPrefix));
      for(int r=0;r<=maxRetryCount;r++){ submitted=g_order.Submit(chosenPlan, risk, ctx, submitExecutionMode, submitAllowLiveExecution, submitAllowDemoExecutionOnly, submitRequireManualExecutionArming, submitManualExecutionArmed, MagicNumber, maxSlippagePoints, TradeCommentPrefix, tstate, execReason); if(submitted) break; if(r<maxRetryCount){ Print("[RETRY][PersonalEA] sym=",symbol," op=submit attempt=",(r+1)," reason=",execReason); Sleep(retryDelaySeconds*1000); } else Print("[RETRY][PersonalEA] sym=",symbol," op=submit exhausted reason=",execReason); }
