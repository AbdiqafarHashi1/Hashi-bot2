...
void ProcessSymbol(const string symbol,const bool isNewBar)
  {
   ...
   int trendAddedToArbitration=0,compressionAddedToArbitration=0,microAddedToArbitration=0;
   for(int vi=0;vi<arb.candidateCount;vi++)
     {
      ...
      if(arb.candidates[vi].strategy==STRATEGY_TREND_CONTINUATION) trendAddedToArbitration++;
      else if(arb.candidates[vi].strategy==STRATEGY_COMPRESSION_BREAKOUT) compressionAddedToArbitration++;
      else if(arb.candidates[vi].strategy==STRATEGY_MICRO_SCALPER) microAddedToArbitration++;
     }
   ...
   g_trendArbAdded += trendAddedToArbitration;
   g_compressionArbValid += compressionValid;
   g_compressionArbAdded += compressionAddedToArbitration;
   ...
   for(int r=0;r<arb.candidateCount;r++)
     {
      int ci=rankIndex[r];
      StrategyCandidate rc=arb.candidates[ci];
      double rr=RRNetAfterSpread(rc.plan,ctx);
      double weightedScore=rc.score.totalScore*RegimeCompatibilityWeight(rc.strategy,regime);
      bool selected=(arb.hasWinner && rc.strategy==arb.winningStrategy && rc.plan.entryPrice==arb.plan.entryPrice);
      Print(StringFormat("[ARBITRATION_CANDIDATE_RANK] strategy=%s valid=%s reason=%s direction=%s entry=%.5f sl=%.5f tp1=%.5f tp2=%.5f rr=%.2f score=%.2f weightedScore=%.2f rank=%d selected=%s",
                         StrategyName(rc.strategy),(rc.isValid?"true":"false"),(rc.rejectReason==""?"none":rc.rejectReason),DirName(rc.plan.direction),rc.plan.entryPrice,rc.plan.stopLoss,rc.plan.takeProfit1,rc.plan.takeProfit2,rr,rc.score.totalScore,weightedScore,r+1,(selected?"true":"false")));
     }
...
void OnDeinit(const int reason){ if(InpVerboseDiagnostics) Print("PersonalEA deinit reason=", reason);
   ...
   Print(StringFormat("[TREND_PIPELINE_VALIDITY_SUMMARY] trendInternalValid=%d trendAddedToArbitration=%d arbitrationValidPlans=%d trendSelected=%d executeSelectedPlanCalled=%d topRejectReason=%s nextHint=%s",
                      g_trendArbValid,g_trendArbAdded,g_pipePlanOk[0],g_trendArbWinner,g_pipelineExecuteSelectedPlanCalled,g_trendTopArbReject,g_arb.TrendForensicNextHintSummary()));
