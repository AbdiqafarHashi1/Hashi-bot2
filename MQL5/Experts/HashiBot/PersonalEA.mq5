...
bool ExecuteSelectedPlan(const TradePlan &plan,string &blocker)
  {
   ...
   if(!g_execSelectedPlanExists){ blocker="NO_SELECTED_PLAN"; stage="SELECTED_PATH"; reason=blocker; PrintFinalDecision(plan,stage,reason,planValid,riskReached,riskApproved,riskReason,portfolioReached,portfolioApproved,portReason,orderValidateReached,orderValidateOk,orderManagerReached,orderAttempted,orderSuccess,retcode,lastErr,bid,ask,volume); return (orderAttempted || orderSuccess); }
   planValid=IsPlanExecutable(plan);
   if(!planValid){ blocker="PLAN_INVALID_SLTP"; stage="PLAN_VALIDATE"; reason=blocker; PrintFinalDecision(plan,stage,reason,planValid,riskReached,riskApproved,riskReason,portfolioReached,portfolioApproved,portReason,orderValidateReached,orderValidateOk,orderManagerReached,orderAttempted,orderSuccess,retcode,lastErr,bid,ask,volume); return (orderAttempted || orderSuccess); }
   ...
   if(plan.stopLoss<=0.0){ blocker="PLAN_INVALID_SLTP"; stage="PLAN_VALIDATE"; reason=blocker; PrintFinalDecision(plan,stage,reason,planValid,riskReached,riskApproved,riskReason,portfolioReached,portfolioApproved,portReason,orderValidateReached,orderValidateOk,orderManagerReached,orderAttempted,orderSuccess,retcode,lastErr,bid,ask,volume); return (orderAttempted || orderSuccess); }
   if(plan.takeProfit1<=0.0){ blocker="PLAN_INVALID_SLTP"; stage="PLAN_VALIDATE"; reason=blocker; PrintFinalDecision(plan,stage,reason,planValid,riskReached,riskApproved,riskReason,portfolioReached,portfolioApproved,portReason,orderValidateReached,orderValidateOk,orderManagerReached,orderAttempted,orderSuccess,retcode,lastErr,bid,ask,volume); return (orderAttempted || orderSuccess); }
   if(!((plan.direction==TRADE_DIR_LONG&&plan.stopLoss<plan.entryPrice&&plan.entryPrice<plan.takeProfit1)||(plan.direction==TRADE_DIR_SHORT&&plan.takeProfit1<plan.entryPrice&&plan.entryPrice<plan.stopLoss))){ blocker="PLAN_INVALID_SLTP"; stage="PLAN_VALIDATE"; reason=blocker; PrintFinalDecision(plan,stage,reason,planValid,riskReached,riskApproved,riskReason,portfolioReached,portfolioApproved,portReason,orderValidateReached,orderValidateOk,orderManagerReached,orderAttempted,orderSuccess,retcode,lastErr,bid,ask,volume); return (orderAttempted || orderSuccess); }
   ...
   if(!g_execRuntimeLimitsApproved){ blocker="PORTFOLIO_REJECTED"; stage="PORTFOLIO"; reason=blocker; PrintFinalDecision(plan,stage,reason,planValid,riskReached,riskApproved,riskReason,portfolioReached,portfolioApproved,portReason,orderValidateReached,orderValidateOk,orderManagerReached,orderAttempted,orderSuccess,retcode,lastErr,bid,ask,volume); return (orderAttempted || orderSuccess); }
   ...
   Print(StringFormat("[ORDER_PAYLOAD_CHECK] symbol=%s type=%s volume=%.2f bid=%.5f ask=%.5f entry=%.5f sl=%.5f tp=%.5f stopLevel=%d freezeLevel=%d minLot=%.2f maxLot=%.2f lotStep=%.2f tradeMode=%d terminalTradeAllowed=%s mqlTradeAllowed=%s accountTradeAllowed=%s payloadOk=%s reason=%s",
   ...
   if(!tradeAllowed){ blocker="TRADE_NOT_ALLOWED"; stage="ORDER_VALIDATE"; reason=blocker; PrintFinalDecision(plan,stage,reason,planValid,riskReached,riskApproved,riskReason,portfolioReached,portfolioApproved,portReason,orderValidateReached,orderValidateOk,orderManagerReached,orderAttempted,orderSuccess,retcode,lastErr,bid,ask,volume); return (orderAttempted || orderSuccess); }
   if(!volumeOk){ blocker="PLAN_INVALID_VOLUME"; stage="ORDER_VALIDATE"; reason=blocker; PrintFinalDecision(plan,stage,reason,planValid,riskReached,riskApproved,riskReason,portfolioReached,portfolioApproved,portReason,orderValidateReached,orderValidateOk,orderManagerReached,orderAttempted,orderSuccess,retcode,lastErr,bid,ask,volume); return (orderAttempted || orderSuccess); }
   ...
   if(orderManagerReached) Print(StringFormat("[ORDERMANAGER_RESULT] attempted=%s success=%s retcode=%d retcodeDescription=%s order=%I64d deal=%I64d lastError=%d",(g_order.LastAttempted()?"true":"false"),(submitted?"true":"false"),retcode,g_order.LastRetcodeDescription(),g_order.LastOrder(),g_order.LastDeal(),lastErr));
   if(!submitted){ blocker="ORDERMANAGER_REJECTED"; stage="ORDER_SUBMIT"; reason=blocker;} else { blocker="SUCCESS"; stage="ORDER_SUBMIT"; reason="SUCCESS"; g_testerOrdersSuccessful++; }
...
void ProcessSymbol(const string symbol,const bool isNewBar)
  {
   ...
   if(!enteredExecuteSelectedPlan)
      Print(StringFormat("[FINAL_DECISION] stage=SELECTED_PATH reason=ORDERMANAGER_NOT_REACHED strategy=%s selected=%d planValid=%s riskReached=%d riskApproved=%d portfolioReached=%d portfolioApproved=%d orderValidateReached=%d orderManagerReached=%d orderAttempted=%d orderSuccess=%d",
                         StrategyName(chosenPlan.strategy),1,(validPlan?"true":"false"),0,(risk.approved?1:0),1,(portfolioOK?1:0),0,0,0,0));
...
void OnDeinit(const int reason){ if(InpVerboseDiagnostics) Print("PersonalEA deinit reason=", reason);
   string topReason=(g_testerOrdersAttempted>0?"ORDERMANAGER_REJECTED":(g_diagWinners>0?"ORDERMANAGER_NOT_REACHED":"NO_SELECTED_PLAN"));
