   ...
   if(risk.approved && validPlan && allowed && portfolioOK && scaleOK && chosenScore >= activeMinScore && (!scalperMode || candidateGradeOK || chosenFromFallback))
     {
      ...
      bool lifecycleCreated=(submitted && g_tracker.RegisterDryRunTrade(tstate));
      if(lifecycleCreated)
        ...
        { g_pipeSubmitRej[sb]++; Print("[PIPE] dryrun_submit ok=false reason=",execReason," strategy=",StrategyName(chosenPlan.strategy)); Print("[LIFECYCLE_CREATE] ok=false reason=unknown_submit_error id=0"); g_order.MarkBlocked(chosenPlan, risk, symbol, tstate, execReason); g_lastCloseTime=TimeCurrent(); }
      else
        { g_pipeSubmitOk[sb]++; g_pipeLifecycleRej[sb]++; Print("[PIPE] dryrun_submit ok=true reason=submitted_no_lifecycle strategy=",StrategyName(chosenPlan.strategy)); Print("[PIPE] lifecycle_created ok=false reason=lifecycle_create_failed strategy=",StrategyName(chosenPlan.strategy)); Print("[LIFECYCLE_CREATE] ok=false reason=lifecycle_create_failed id=0"); g_order.MarkBlocked(chosenPlan, risk, symbol, tstate, "lifecycle_create_failed"); g_lastCloseTime=TimeCurrent(); }
     }
   else
     {
      string breason=(!validPlan?"invalid_plan":(!risk.approved?"risk_not_approved":(!portfolioOK?"portfolio_not_approved":(!allowed?guard:(!scaleOK?scaleReason:"unknown_submit_error"))))); g_pipeSubmitRej[sb]++; if(validPlan && risk.approved && portfolioOK && allowed && !scaleOK){} else g_pipeLifecycleRej[sb]++; Print("[PIPE] dryrun_submit ok=false reason=",breason," strategy=",StrategyName(chosenPlan.strategy)); Print("[PIPE] lifecycle_created ok=false reason=",breason," strategy=",StrategyName(chosenPlan.strategy)); Print("[LIFECYCLE_CREATE] ok=false reason=",breason," id=0"); g_order.MarkBlocked(chosenPlan, risk, symbol, tstate, breason); Print("[SCALE] rejected reason=",breason," sym=",symbol); g_lastCloseTime=TimeCurrent();
     }
