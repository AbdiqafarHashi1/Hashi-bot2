...
bool RunDryRunSelfCheck()
  {
   ...
   ok = (g_regime.Init() && ok);
   ok = (g_arb.Init() && ok);
   ok = (g_risk.Init(PROFILE_PROP_FIRM) && ok);
   ok = (g_prop.Init() && ok);
   ok = (g_order.Init(true) && ok);
   ok = (g_tracker.Init() && ok);

   Print("[SelfCheck][PropFirmEA] ", (ok ? "PASS" : "FAIL"), " (dry-run only, live execution disabled)");
   return ok;
  }
...
int OnInit()
  {
   ...
   g_regime.Init();
   g_arb.Init();
   g_risk.Init(PROFILE_PROP_FIRM);
   g_prop.Init();
   g_order.Init(true);
   g_tracker.Init();
   Print("PropFirmEA initialized");
   ...
   if(g_ctxBuilder.Build(_Symbol, contextTimeframe, ctx))
     {
      ...
      RiskDecision risk;
      g_risk.AssessWithProp(arb, ctx, g_prop, risk);
      TradeState tstate;
      string vreason = "";
      ...
      Print(g_ctxBuilder.Describe(ctx), " | ", g_regime.Describe(regime), " | ", g_arb.Describe(arb), " | ", g_risk.Describe(risk), " | ", g_prop.Describe(), " | valid=", vreason, " | ", g_order.DescribeLastAction(), " | ", g_tracker.Describe());
     }
   else
...
void OnDeinit(const int reason)
  {
   g_prop.SaveState();
   Print("PropFirmEA deinit reason=", reason);
  }
