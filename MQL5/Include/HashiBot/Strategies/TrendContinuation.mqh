   ...
   bool Analyze(const MarketContext &ctx,const RegimeState &regime,StrategyCandidate &candidate)
     {
      ...
      if(!structureOK && testerMode)
        {
         bool emaTrendProxy=(dir==TRADE_DIR_LONG ? (ctx.emaFast>=ctx.emaSlow && ctx.currentClose>=ctx.emaFast-0.25*ctx.atr) : (ctx.emaFast<=ctx.emaSlow && ctx.currentClose<=ctx.emaFast+0.25*ctx.atr));
         if(emaTrendProxy){ structureOK=true; structureScore=0.52; }
        }
      if(!structureOK)
        {
         m_audit.lastRejectReason="STRUCTURE_NOT_FOUND";
      ...
      bool setupPass=(directionPass && (momentumPathOk || reclaimPathOk) && structureOK);
