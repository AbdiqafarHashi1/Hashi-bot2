   ...
   bool Analyze(const MarketContext &ctx,const RegimeState &regime,StrategyCandidate &candidate)
     {
      ...
      if(!DetectBox(ctx, boxHigh, boxLow, boxWidth, boxAge, insideRatio, touchScore))
        {
         if(testerMode && ctx.barsLoaded>=8)
           {
            boxAge=MathMin(ctx.barsLoaded,10);
            boxHigh=ctx.recentHigh[1]; boxLow=ctx.recentLow[1];
            for(int bi=2;bi<boxAge;bi++){ if(ctx.recentHigh[bi]>boxHigh) boxHigh=ctx.recentHigh[bi]; if(ctx.recentLow[bi]<boxLow) boxLow=ctx.recentLow[bi]; }
            boxWidth=boxHigh-boxLow;
            insideRatio=0.20; touchScore=0.05;
           }
         else
