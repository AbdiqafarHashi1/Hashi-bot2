...
export async function POST() {
  ...
  if (!prisma) {
    ...
    if (!existing.isRunning) return NextResponse.json({ message: "already_stopped", control: existing }, { status: 409 });
    const control = await writeSystemControlFile({ isRunning: false });
    return NextResponse.json({ control });
  }
  const existing = await ensureControlRow(prisma);
  if (!existing) {
    const fallback = await readSystemControlFile();
    if (!fallback.isRunning) return NextResponse.json({ message: "already_stopped", control: fallback }, { status: 409 });
    const control = await writeSystemControlFile({ isRunning: false });
    return NextResponse.json({ control });
  }
  if (!existing.isRunning) return NextResponse.json({ message: "already_stopped", control: existing }, { status: 409 });
