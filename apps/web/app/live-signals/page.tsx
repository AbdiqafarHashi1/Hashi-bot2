...
export default function Page() {
  ...
  const feedSummary = useMemo(() => {
    if (!data) return { healthy: 0, stale: 0, blocked: 0, ws: 0, restFallback: 0, marketClosed: 0 };
    return data.symbolScanBoard.reduce((acc, entry) => {
      if ((entry.blockedReason ?? "").includes("feed_stale")) acc.stale += 1;
      if ((entry.blockedReason ?? "").includes("forex_market_closed")) acc.marketClosed += 1;
      if (entry.blockedReason) acc.blocked += 1;
      else acc.healthy += 1;
      if ((entry.activeProvider ?? "").includes("ws")) acc.ws += 1;
      if ((entry.fallbackStatus ?? "").includes("fallback")) acc.restFallback += 1;
      return acc;
    }, { healthy: 0, stale: 0, blocked: 0, ws: 0, restFallback: 0, marketClosed: 0 });
  }, [data]);
  ...
  return (
    <section className="space-y-4 sm:space-y-5">
      ...
      <section className="rounded border border-slate-800 bg-slate-900/70 p-3 text-xs text-slate-300">Feed summary · healthy: {feedSummary.healthy} · stale: {feedSummary.stale} · blocked: {feedSummary.blocked} · forex market closed: {feedSummary.marketClosed} · active websocket connections: {feedSummary.ws} · symbols on REST fallback: {feedSummary.restFallback}</section>
